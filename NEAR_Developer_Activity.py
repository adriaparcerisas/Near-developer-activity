...
st.write('But, first of all, what are developers? Well, _developers are people that can work on a specific or multiple projects in a sporadic, part or full-time job dedicated to storing, tracking, and collaborating on software projects._')
st.write('Developers can create new projects from the scratch or basically take part from an existing one. If its a new project developers create a new _repository_ where the full project will be developed there. Afterthat, several _Pull Requests_ (PRs) will be carried out there representing the different project steps.')
st.write('For this reason, other important metrics to be analyzed are the evolution of repositories and the amount of PRs.')
...
WITH
  ...
  from near.beta.github_activity
    where updatedat between CURRENT_DATE-INTERVAL '7 MONTHS' and CURRENT_DATE-INTERVAL '6 MONTHS'
  group by 1
  ),
  users_retention as (
SELECT
